...
def optimize():
    ...
    try:
        ...
        max_weight = float(data.get('max_weight', 0.17))  # VP-approved conservative limit
...
def optimize_walkforward():
    """
    Phase 2 & Phase 3: Walk-forward backtesting with optional ML predictions
    
    When use_predictive=False (default): Phase 2 - Historical means
    When use_predictive=True: Phase 3 - ML-based return forecasts
    """
    try:
        data = request.json
        ...
        initial_capital = data.get('initial_capital', 100000)
        benchmark_name = data.get('benchmark', 'NIFTY50')
        max_weight = float(data.get('max_weight', 0.17))  # VP-approved: conservative 17%
        ...
        # Phase 3 predictive parameters (optional)
        use_predictive = data.get('use_predictive', False)  # Toggle for Phase 3
        ...
        # Step 1: Fetch data
        mode_label = "PREDICTIVE ML" if use_predictive else "WALK-FORWARD"
        print(f"\n{'='*80}")
        print(f"{mode_label} OPTIMIZATION REQUESTED")
        print(f"{'='*80}")
        
        if use_predictive:
            print(f"Mode: Phase 3 - ML Forecasting")
            ...
            print(f"Mode: Phase 2 - Historical Means")
        ...
        # Step 2: Configure and run appropriate engine
        config = {
            'lookback_months': lookback_months if not use_predictive else alpha_lookback,
        ...
        if use_predictive:
            # Phase 3: Predictive Walk-Forward
            print("\n🤖 Initializing Predictive ML Engine...")
            ...
            # Phase 2: Standard Walk-Forward
            print("\n📋 Initializing Standard Walk-Forward Engine...")
        ...
        response = {
            "success": True,
            "mode": "predictive_ml" if use_predictive else "walkforward",
            "strategy": results,
            "benchmark": {
                "selected": benchmark_name,
                "index": benchmark_metrics,
            ...
            "message": f"{'Predictive ML' if use_predictive else 'Walk-forward'} optimization completed successfully"
        }
        
        # Add Phase 3 specific config and diagnostics
        if use_predictive:
            ...
            # Add ML diagnostics if available
            if 'predictive_diagnostics' in results:
                response['ml_diagnostics'] = results['predictive_diagnostics']
